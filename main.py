...
def _review_wp_article(original_article: dict, analysis: dict, primary: dict, related: dict | None, config: dict, is_rerun: bool) -> dict:
    while True:
        try:
            model = "gemini-exp-1206"  # Use high quality model for articles
            if is_rerun:
                model = "gemini-2.0-flash-exp"  # Use cheaper model for reruns
...
def main() -> None:
    ...
    if generate_article == "y":
        if args.non_interactive:
            ...
            model_name = "gemini-exp-1206"  # default (high quality)
            try:
                ...
                if active_profile:
                    ...
                    for key in ["PROMPT_ARTICLE_SYSTEM", "PROMPT_ARTICLE_USER"]:
                        if key in profile_prompts and isinstance(profile_prompts[key], dict):
                            model_name = profile_prompts[key].get("model", "gemini-exp-1206")
                            break
            except Exception:
